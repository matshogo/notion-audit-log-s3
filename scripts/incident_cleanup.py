...
def cleanup_s3(s3, bucket: str, start: date, end: date, execute: bool) -> None:
    ...
    for d in date_range(start, end):
        ...
        for tmpl in PREFIX_TEMPLATES:
            prefix = tmpl.format(
                y=d.strftime("%Y"), m=d.strftime("%m"), d=d.strftime("%d")
            )
    ...
    print()
    print(
        f"合計: {total_count:,} 件（バージョン・削除マーカー含む） / {total_bytes / 1024 / 1024:,.1f} MB"
    )
    if execute:
...
def set_log_retention(logs, retention_days: int, execute: bool) -> None:
    print()
    print(
        f"CloudWatch Logs 保持期間: {retention_days} 日 {'（設定実行）' if execute else '（dry-run）'}"
    )
    for group in LOG_GROUPS:
        ...
        try:
            logs.put_retention_policy(
                logGroupName=group, retentionInDays=retention_days
            )
            print(f"  {group}: 設定完了")
...
def main() -> int:
    ...
    parser.add_argument(
        "--bucket", help="対象バケット名（省略時: notion-audit-logs-<account-id>）"
    )
    parser.add_argument(
        "--start",
        default="2026-07-08",
        help="削除対象の開始日 YYYY-MM-DD（既定: 2026-07-08 = ループ開始日）",
    )
    parser.add_argument(
        "--end",
        default=date.today().isoformat(),
        help="削除対象の終了日 YYYY-MM-DD（既定: 今日）",
    )
    parser.add_argument(
        "--retention-days",
        type=int,
        default=1,
        help="CloudWatch Logs の保持日数（既定: 1）",
    )
    parser.add_argument(
        "--skip-s3", action="store_true", help="S3クリーンアップをスキップ"
    )
    parser.add_argument(
        "--skip-logs", action="store_true", help="ログ保持期間の設定をスキップ"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
        help="実際に削除・設定を行う（省略時は dry-run）",
    )
