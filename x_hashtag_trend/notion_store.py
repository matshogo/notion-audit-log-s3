...
def get_notion_config() -> tuple[str, str]:
    ...
    if not token:
        ...
        print(
            "Notion Integration を作成してトークンを取得してください:", file=sys.stderr
        )
        print("  https://www.notion.so/my-integrations", file=sys.stderr)
        sys.exit(1)
    if not db_id:
        print(
            "エラー: 環境変数 NOTION_DATABASE_ID が設定されていません。",
            file=sys.stderr,
        )
        sys.exit(1)
...
def ensure_database_schema(token: str, db_id: str) -> None:
    ...
    try:
        ...
        print(
            f"Notion DB: {db.get('title', [{}])[0].get('plain_text', 'Untitled')}",
            file=sys.stderr,
        )
...
def store_analysis(hashtag: str, analysis: dict, report_text: str) -> str:
    ...
    co_tags = ", ".join(
        ...
        for item in analysis.get("co_hashtags", [])[:10]
    )
    keywords = ", ".join(
        f"{item['word']}({item['count']})"
        for item in analysis.get("keyword_freq", [])[:15]
    )
    ...
    properties = {
        "Name": {
            "title": [
                {
                    "text": {
                        "content": f"#{hashtag} トレンド分析 {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')}"
                    }
                }
            ],
        },
    ...
    report_blocks.append(
        {
            ...
            "heading_2": {
                "rich_text": [
                    {
                        "type": "text",
                        "text": {"content": f"#{hashtag} トレンド分析レポート"},
                    }
                ],
            },
        }
            ...
            report_blocks.append(
                {
                    ...
                    "paragraph": {
                        "rich_text": [
                            {"type": "text", "text": {"content": tweet_text}}
                        ],
                    },
                }
            )
